test_that("get_airnow_conditions() catches invalid inputs", {
  httptest2::without_internet({
    ...
    expect_error(get_airnow_conditions(zip = "98101", distance = NA_real_), "distance") # nolint
    expect_error(get_airnow_conditions(zip = "98101", distance = -30), "distance") # nolint
    expect_error(get_airnow_conditions(zip = "98101", distance = 1:5), "distance") # nolint

    # clean_names is a logical scalar
    expect_error(get_airnow_conditions(zip = "98101", clean_names = NULL), "clean_names") # nolint
    expect_error(get_airnow_conditions(zip = "98101", clean_names = NA), "clean_names") # nolint
    expect_error(get_airnow_conditions(zip = "98101", clean_names = 1), "clean_names") # nolint
    expect_error(get_airnow_conditions(zip = "98101", clean_names = c(TRUE, FALSE)), "clean_names") # nolint
  })
})
